...
function adduser(){
    ...
    read -p "(Default password: defpasswd):" shadowsockspwd
    [ -z "$shadowsockspwd" ] && shadowsockspwd="defpasswd"
    echo
    echo "---------------------------"
	...
	#添加用户
	cd ${ssrdir}
	python mujson_mgr.py -a -p ${shadowsocksport} -k ${shadowsockspwd} -m aes-256-cfb -O auth_sha1_v4_compatible -o tls1.2_ticket_auth_compatible
